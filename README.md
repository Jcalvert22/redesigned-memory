...
|-----------|------------|------|-------------|
| Authenticaiton System | Added supabase capability for authentication | https://supabase.com/dashboard/project/itledwbcorbmpeogbtgn/auth/users?filter=id  | Adding supabase was easy we just implemented the ANON key into our .env on our vm and it was seamless from there |
| Database Upgrade | The Supabase implementation doubles as a database for storing the different username and passwords they are hashed so the owner of the database cannot get any passwords | https://supabase.com/dashboard/project/itledwbcorbmpeogbtgn/database/schemas | Being able to store the username and passwords help registering and signing in faster and more effecience |
...
| Search funcitonality | Implemented a search bar to filter/select muscle groups | https://github.com/Jcalvert22/redesigned-memory/blob/main/search.html | We wanted to add this funcitonality to make it easier to see what exercises are in the app or what needs to be added |
| User feedback System | This is an area on the bar where people can leave reviews of the app | https://github.com/Jcalvert22/redesigned-memory/blob/main/feedback.html | This helps people see what people think about the app |
...
| Debug Case Study | We kept running into problems with our Ci/Cd file | https://github.com/Jcalvert22/redesigned-memory/blob/main/docs/debug_case_study.md | The problem was ownership issues in the vm itself |
| Monitoring and Logging | Implemented with NetStat | http://34.16.191.24:19999/#metrics_correlation=false&after=-900&before=0&modal=&modalTab= | This allows to see activity on the vm and how heavy it is running |
| Automation Ci/Cd | Implemented so we did not have to manually pull everything on the repo | https://github.com/Jcalvert22/redesigned-memory/actions/runs/24848723761  | After we fixed the ownership issues this made it easiest to run |
| Deployment Guide | These are the exact steps we used to start up our VM | https://github.com/Jcalvert22/redesigned-memory/blob/main/docs/deploymentGuide.md | The deployment guide will help if the vm becomes corrupted and we need to restart from scratch |
